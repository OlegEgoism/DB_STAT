...
function initDashboardActions() {
    const actionHandlers = {
        'open-connection-modal': openConnectionModal,
    ...
    document.querySelectorAll('[data-action]').forEach(element => {
        ...
        if (action === 'change-connection') {
            element.addEventListener('change', event => onConnectionChange(event.target.value));
            return;
        }

        if (actionHandlers[action]) {
            element.addEventListener('click', actionHandlers[action]);
        }
    });

    document.querySelectorAll('[data-toast-message]').forEach(element => {
        element.addEventListener('click', () => showToast(element.dataset.toastMessage));
    });
}
