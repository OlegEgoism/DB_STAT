...
from db_statistics.models import DBAudit, DBConnection, DBUser, DBUserSidebarSettings
...
class SidebarFavoritesSettingsTests(TestCase):
    ...
    def test_user_can_reorder_sidebar_sections(self):
        section_order = ["additional", "administration", "performance", "data", "infrastructure"]

        response = self.client.post(reverse("sidebar_settings"), data=json.dumps({"visible_tabs": ["tables", "audit"], "section_order": section_order}), content_type="application/json")

        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json()["section_order"], section_order)
        preferences = DBUserSidebarSettings.objects.get(user=self.user).visible_tabs
        self.assertEqual(preferences, {"visible_tabs": ["tables", "audit"], "section_order": section_order})
        self.assertIn("Порядок блоков: Дополнительно, Администрирование, Производительность, Данные, Инфраструктура", DBAudit.objects.get(action_type="sidebar_settings").info)

    def test_legacy_sidebar_settings_receive_default_section_order(self):
        DBUserSidebarSettings.objects.create(user=self.user, visible_tabs=["tables", "audit"])

        response = self.client.get(reverse("sidebar_settings"))

        self.assertEqual(response.json()["visible_tabs"], ["tables", "audit"])
        self.assertEqual(response.json()["section_order"], ["infrastructure", "data", "performance", "administration", "additional"])

    def test_legacy_request_does_not_reset_saved_section_order(self):
        section_order = ["additional", "data", "infrastructure", "performance", "administration"]
        DBUserSidebarSettings.objects.create(user=self.user, visible_tabs={"visible_tabs": ["tables", "audit"], "section_order": section_order})

        response = self.client.post(reverse("sidebar_settings"), data=json.dumps({"visible_tabs": ["tables"]}), content_type="application/json")
