...
## Лицензия коммерческой версии

При первом запуске DB STAT перенаправляет пользователя на `/license/` и не
открывает остальные страницы, пока не будет загружен действующий подписанный
файл лицензии.

### Интерактивное создание лицензии

Чтобы не передавать все параметры вручную, запустите мастер из корня проекта:

```bash
python scripts/create_license.py
```

Мастер запросит только название организации, первый и последний дни действия.
Подписанный `.license`-файл автоматически сохраняется в каталоге `scripts`.
При первом запуске мастер сам создаёт постоянную пару Ed25519: открытый ключ — в
`license-public.pem`, закрытые данные издателя — в игнорируемом Git каталоге
`scripts/.license-issuer`. Сохраните резервную копию этого каталога отдельно от
проекта: без неё продлить ранее выданные лицензии тем же ключом не получится.

Файл лицензии содержит только пять необходимых полей: организацию, даты начала
и окончания, уникальный хеш активации и цифровую подпись. Служебные поля Django
или данные закрытого ключа в клиентский файл не записываются.

Пара ключей создаётся один раз. Закрытый ключ храните только у издателя и не
добавляйте его в Git или Docker-образ:

```bash
export LICENSE_PRIVATE_KEY_PASSWORD='надежный-пароль-из-хранилища-секретов'
python manage.py generate_license_keys \
  --private-key /secure/db-stat-private.pem \
  --public-key ./license-public.pem
```

Выпуск лицензии для организации с заданным периодом действия:
...
Команда выводит уникальный SHA-256 хеш активации и записывает его внутрь
подписанного файла. Укажите путь к открытому ключу в
`LICENSE_PUBLIC_KEY_FILE`. Клиент активирует приложение загрузкой полученного
`.license`-файла на стартовой странице. Дата `valid-until` является последним
действующим календарным днём в UTC: лицензия по `2026-01-01` будет отклонена с
`2026-01-02 00:00:00 UTC`.

Для Docker передайте открытый ключ только для чтения, а файл установленной
лицензии будет сохранён в подключённом томе `/app/data`:

```bash
docker run --name db-stat --rm -p 8000:8000 \
  -v db-stat-data:/app/data \
  -v ./license-public.pem:/run/secrets/license-public.pem:ro \
  -e LICENSE_PUBLIC_KEY_FILE=/run/secrets/license-public.pem \
  db-stat
```
