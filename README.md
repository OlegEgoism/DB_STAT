...
## Как устроен и работает проект

DB STAT разделяет **служебные данные приложения** и **данные наблюдаемых СУБД**. Пользователи, сессии, настройки интерфейса, избранное, зашифрованные реквизиты подключений, задачи обслуживания и аудит находятся в SQLite. Метрики и сведения об объектах не копируются в SQLite: Django запрашивает их непосредственно из выбранной PostgreSQL-совместимой БД и возвращает браузеру в JSON.

![Графическая схема работы DB STAT: пользователь, Django, SQLite, пул соединений, целевая СУБД и фоновые задачи](docs/architecture.ru.svg)

На схеме сплошными стрелками показан основной путь запроса: браузер обращается к Django, приложение проверяет сессию, роль и доступ в SQLite, получает соединение из пула и выполняет параметризованный SQL-запрос к выбранной СУБД. Результат возвращается в интерфейс как HTML или JSON, а соединение — обратно в пул.

Пунктирными стрелками показаны управляющие операции. После проверки прав Django создаёт задачу обслуживания, фоновый исполнитель запускает `VACUUM`, `ANALYZE` или `EXPLAIN ANALYZE`, а статус, результат и событие аудита сохраняются во внутренней SQLite. Операции, изменяющие состояние целевой БД или завершающие процессы, доступны только роли **Администратор**.

> SQLite — внутренняя БД DB STAT, а PostgreSQL, Greenplum и Greengage — внешние цели мониторинга. Пароль внешнего подключения хранится в SQLite в зашифрованном виде и расшифровывается только на стороне приложения при создании соединения. Ошибки драйвера подробно записываются в серверный журнал, а клиент получает безопасное сообщение без раскрытия внутренней структуры БД.
