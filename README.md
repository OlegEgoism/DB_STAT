...
В поле «Хост» указывайте привычный адрес `localhost` (также поддерживаются `127.0.0.1` и `::1`). Приложение сохраняет введённое понятное пользователю значение, но непосредственно перед подключением внутри Docker автоматически заменяет loopback-адрес на значение `LOCAL_DATABASE_HOST` (`host.docker.internal`). Для удалённой БД укажите её настоящее DNS-имя или IP — такие адреса не изменяются.
