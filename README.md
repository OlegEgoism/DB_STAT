...
Во время сборки образа применяются миграции и создаётся администратор `admin` с паролем `admin`. При старте контейнер повторно применяет миграции, проверяет наличие этого администратора и запускает Django на `0.0.0.0:8000`. Для сохранения SQLite-базы между перезапусками подключите volume, например:
