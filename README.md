...
В поле «Хост» указывайте привычный адрес `localhost` (также поддерживаются `127.0.0.1` и `::1`) — вводить `host.docker.internal` вручную не нужно. Приложение сохраняет введённое понятное пользователю значение, но непосредственно перед подключением внутри Docker автоматически заменяет loopback-адрес на `host.docker.internal`. Это работает и без явно заданной переменной `LOCAL_DATABASE_HOST`: приложение определяет запуск в контейнере по `/.dockerenv` или `/run/.containerenv`. Переменная нужна только для переопределения адреса. Для удалённой БД укажите её настоящее DNS-имя или IP — такие адреса не изменяются.

Если сообщение об ошибке всё ещё содержит `127.0.0.1`, запущен **старый образ**: исправленная версия при фактическом подключении из контейнера обращается к `host.docker.internal`, хотя в форме отображается `localhost`.
