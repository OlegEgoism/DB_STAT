...
- Автоматический запуск ruff перед коммитом и перед push (только на ветке `dev`) — один раз после клонирования:
...
Дальше на ветке `dev`:
- каждый `git commit` прогоняет `ruff check --fix`/`ruff format` по застейдженным
  `.py`-файлам (`.githooks/pre-commit`). Если ruff что-то поправил, коммит прерывается,
  а исправления уже застейджены — посмотри `git diff --cached` и закоммить ещё раз.
  Частично застейдженные файлы (застейджена только часть правок) не трогает — во
  избежание риска задеть незастейдженный код.
- каждый `git push` дополнительно проверяет (без авто-исправления — коммиты уже
  созданы) весь проект целиком: `ruff check .` и `ruff format --check .`
  (`.githooks/pre-push`) — то же самое, что проверяет CI, только раньше и локально.

На `main` и остальных ветках оба хука ничего не делают — там форматирование проверяет
CI при пуше.
