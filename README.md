...
Если нужно подключаться из приложения к PostgreSQL/Greenplum, который запущен локально на этой же машине, запускайте контейнер в host-сети. Тогда значение `localhost` в поле **Хост** внутри формы подключения будет означать локальную БД на вашей машине:
...
Обычный запуск через проброс порта подходит, если приложению не нужно подключаться к локальной БД хост-машины по `localhost`:

```bash
docker run --rm -p 8000:8000 db-stat
```

Почему так: при запуске с `-p 8000:8000` контейнер работает в отдельной bridge-сети, и `localhost` внутри контейнера указывает на сам контейнер, а не на вашу машину. Поэтому для сценария «в форме подключения указать `localhost` и попасть в локальную БД» нужен `--network=host`.

Если после обновления вы всё ещё видите ошибку подключения к `172.17.0.1` или `192.168.0.1`, значит запущен старый Docker-образ с прежней логикой подмены `localhost`. Пересоберите образ и запустите контейнер заново:

```bash
docker build --no-cache -t db-stat .
docker run --rm --network=host db-stat
```
